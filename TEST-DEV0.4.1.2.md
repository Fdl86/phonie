...
```text
DEV0.4.1.2 - SIA RADIO & CONTROLLER VOICES
```
...
La génération doit afficher séparément le résultat des deux méthodes. L'un des deux scénarios est acceptable :

- le catalogue segmenté détecte directement au moins 200 documents ;
- le catalogue est déclaré inutilisable, puis le DVD AIRAC direct détecte au moins 200 documents.

La base finale doit ensuite contenir plus de 200 aérodromes et plus de 150 fréquences. Elle ne doit plus s'arrêter sur `Catalogue VAC incomplet: seulement 0 documents`.
